...
namespace Labyrinth {
	...
	private:
		...
		static void GetTileData(const std::filesystem::path& tileset, size_t firstID, std::vector<SheetData>& setData);
		static void GetTileData(rapidxml::xml_node<char>* tilesetNode, size_t firstID, const std::filesystem::path& pngPath, std::vector<SheetData>& setData);
