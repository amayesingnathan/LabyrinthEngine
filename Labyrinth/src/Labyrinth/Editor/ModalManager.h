...
namespace Laby {
    ...
    public:
        ...
        static void OpenInline(std::string_view title, ModalButtons type, Action<>&& OnRender, Action<>&& OnComplete = [] {}) { Open<InlineModal>(title, type, std::move(OnRender), std::move(OnComplete)); }
