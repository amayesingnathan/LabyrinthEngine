...
namespace Laby {
	...
	public:
		...
		static void SetField(UUID entID, std::string_view field, const T& val)
		{
		...
		static void UnsetField(UUID entID, std::string_view field);

		static FieldValues& GetFields(UUID entID) { return sCachedFields[entID]; }

		template<IsFieldType T>
		static T& GetFieldValue(UUID entID, std::string_view fieldName)
		{
